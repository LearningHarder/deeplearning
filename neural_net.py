...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0,testflag = False):
        """
        ...
        loss = 0.0
        row_max  = np.max(scores,1) #  vector
        e_x = np.exp(scores.T - row_max).T # N * C
        p_mat = (e_x.T / np.sum(e_x,1)).T # N * C
        loss += -np.sum(np.log(p_mat[range(N),y]))
        # avg
        loss /= N
        loss += reg * np.sum(W1 *W1) + reg * np.sum(W2 *W2) 
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        if testflag:return loss 
        ...
        dscores = p_mat.copy()
        dscores[range(N), list(y)] -= 1 # 每个分量求导
        dscores /= N
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            test_loss = self.loss(X_val, y=y_val, reg=reg,testflag= True)
